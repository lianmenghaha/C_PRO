                ...
                if '.svg' in svg:
                    ...
                    for path in paths:
                        ...
                        while (flag == 1):
                            ...
                            curMicdistinctpaths = curcombinedPaths[:]#now curcombinedPaths contains all the rectangles
                            flag = 0
                            for path1 in curMicdistinctpaths:
                                for path2 in curMicdistinctpaths:
                                    # LM:name
                    ...
                    # for i in range(0, length):
                    # for j in range(i+1, length):
                    # if distinctPaths[i].name in no_absorption.keys():
                    # if distinctPaths[j].name in no_absorption[distinctPaths[i].name]:
                    # break
                    # conflict, line = lp_conflict(distinctPaths[i], distinctPaths[j], lp_factor)
                    # if conflict:
                    # if distinctPaths[i].name not in no_absorption.keys():
                    # no_absorption[distinctPaths[i].name] = []
                    # if distinctPaths[j].name not in no_absorption[distinctPaths[i].name]:
                    # no_absorption[distinctPaths[i].name].append(distinctPaths[j].name)
                    # if distinctPaths[j].name not in no_absorption.keys():
                    # no_absorption[distinctPaths[j].name] = []
                    # if distinctPaths[i].name not in no_absorption[distinctPaths[j].name]:
                    # no_absorption[distinctPaths[j].name].append(distinctPaths[i].name)
